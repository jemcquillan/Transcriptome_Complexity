...
novel_EpT <- list.files('deuterostome_complexity_stats/deuterostome_novel_genes/deuterostome_novel_EpT/')
novel_TpG_EpG <- list.files('deuterostome_complexity_stats/deuterostome_novel_genes/deuterostome_novel_TpG_EpG//')

# Creates a list of files from the directory that has the appropriate metrics of orthologs
ortho_EpT <- list.files('deuterostome_complexity_stats/deuterostome_ortholog/deuterostome_ortholog_EpT/')
ortho_TpG_EpG <- list.files('deuterostome_complexity_stats/deuterostome_ortholog/deuterostome_ortholog_TpG_EpG/')

# Creates a list of files from the directory that has the appropriate metrics of whole transcriptomes
wg_EpT <- list.files('deuterostome_complexity_stats/deuterostome_whole_genome/deuterostome_whole_genome_EpT/')
wg_TpG_EpG <- list.files('deuterostome_complexity_stats/deuterostome_whole_genome/deuterostome_whole_genome_TpG_EpG/')
...
output_directory <- "<your path here>/deuterostome_complexity_stats/Deuterostome_density_plots"
...
for(i in 1:length(ortho_EpT)){
  ...
  ortho_EpT_df <- read.csv(file.path('deuterostome_complexity_stats/deuterostome_ortholog/deuterostome_ortholog_EpT/',ortho_EpT[i]), sep = ',')
  ...
  ortho_TpG_EpG_df <- read.csv(file.path('deuterostome_complexity_stats/deuterostome_ortholog/deuterostome_ortholog_TpG_EpG/',ortho_TpG_EpG[i]), sep = ',')
  ...
  wg_EpT_df <- read.csv(file.path('deuterostome_complexity_stats/deuterostome_whole_genome/deuterostome_whole_genome_EpT/',wg_EpT[i]), sep = ',')
  wg_EpT_df$group <- 'whole transcriptome'
  
  wg_TpG_EpG_df <- read.csv(file.path('deuterostome_complexity_stats/deuterostome_whole_genome/deuterostome_whole_genome_TpG_EpG/',wg_TpG_EpG[i]), sep = ',')
  wg_TpG_EpG_df$group <- 'whole transcriptome'
  
  # This creates a dataframe with all the data from each EpT metric file for novel genes
  novel_EpT_df <- read.csv(file.path('deuterostome_complexity_stats/deuterostome_novel_genes/deuterostome_novel_EpT/',novel_EpT[i]), sep = ',')
  # This creates and appends a column names group, and labels the orthologs 'ortho'
  ...
  # Same this here as above. Creates a data frame with all the data, and adds column group
  # labeled ortho for all the orthologs for the other 2 metrics
  novel_TpG_EpG_df <- read.csv(file.path('deuterostome_complexity_stats/deuterostome_novel_genes/deuterostome_novel_TpG_EpG/',novel_TpG_EpG[i]), sep = ',')
  ...
  novel_ept_plot_trunc <- ggplot(novel_EpT_df, aes_string('num_exon', fill = 'group', color = 'group')) +
    ...
    labs(title = paste0(file_name), subtitle = paste0('Novel Genes'), x = 'EpT')
  ...
  novel_epg_plot_trunc <- ggplot(novel_TpG_EpG_df, aes_string('num_uniq_exon', fill = 'group', color = 'group')) +
    geom_density(bw = "bcv",color=3,fill=3,alpha=0.4) + xlim(0,30) +
    labs(title = paste0(file_name), subtitle = paste0('Novel Genes'), x = 'EpG')
  ...
  #Combine  the whole transcriptome and orthologs into one df with the metric EpT
  EpT_df <- rbind(wg_EpT_df,ortho_EpT_df)
  EpT_df$group <- factor(EpT_df$group, levels = c("ortho", "whole transcriptome"))

  #K-S Between EpT of ortho vs wg
  ...
  #comine TpG_EpG_df
  TpG_EpG_df <- rbind(ortho_TpG_EpG_df, wg_TpG_EpG_df)

  #K-S Between EpG of ortho vs wg
  ...
  #K-S Between TpG of ortho vs wg
...
TpG_Bon_Fisher_meth = fisher(p.adjust(pvals_df$p_value_TPG, method = "bonferroni"))
...
# Print plots into PDFs

pdf(paste0(output_directory,'/Deuterostome_EpT_plot_novel_trunc.pdf'),7,5)
for(i in 1:length(ortho_EpT)){
  print(plot_novel_EpT_trunc[[i]])
}
dev.off()

pdf(paste0(output_directory,'/Deuterostome_EpT_plot_novel.pdf'),7,5)
for(i in 1:length(ortho_EpT)){
  print(plot_novel_EpT[[i]])
}
dev.off()

pdf(paste0(output_directory,'/Deuterostome_TpG_plot_novel_trunc.pdf'),7,5)
for(i in 1:length(ortho_EpT)){
  print(plot_novel_TpG_trunc[[i]])
}
dev.off()

pdf(paste0(output_directory,'/Deuterostome_TpG_plot_novel.pdf'),7,5)
for(i in 1:length(ortho_EpT)){
  print(plot_novel_TpG[[i]])
}
dev.off()

pdf(paste0(output_directory,'/Deuterostome_EpG_plot_novel_trunc.pdf'),7,5)
for(i in 1:length(ortho_EpT)){
  print(plot_novel_EpG_trunc[[i]])
}
dev.off()

pdf(paste0(output_directory,'/Deuterostome_EpG_plot_novel.pdf'),7,5)
for(i in 1:length(ortho_EpT)){
  print(plot_novel_EpG[[i]])
}
dev.off()

pdf(paste0(output_directory,'/Deuterostome_EpT_plot_wilcoxon.pdf'),7,5)
for(i in 1:length(ortho_EpT)){
  print(plot_EpT[[i]])
}
dev.off()

pdf(paste0(output_directory,'/Deuterostome_EpG_plot_wilcoxon.pdf'),7,5)
for(i in 1:length(ortho_EpT)){
  print(plot_EpG[[i]])
}
dev.off()

pdf(paste0(output_directory,'/Deuterostome_TpG_plot_wilcoxon.pdf'),7,5)
for(i in 1:length(ortho_EpT)){
  print(plot_TpG[[i]])
}
dev.off()

pdf(paste0(output_directory,'/Deuterostome_EpT_truncated_plot_wilcoxon.pdf'),7,5)
for(i in 1:length(ortho_EpT)){
  print(plot_EpT_trunc[[i]])
}
dev.off()

pdf(paste0(output_directory,'/Deuterostome_EpG_truncated_plot_wilcoxon.pdf'),7,5)
for(i in 1:length(ortho_EpT)){
  print(plot_EpG_trunc[[i]])
}
dev.off()

pdf(paste0(output_directory,'/Deuterostome_TpG_truncated_plot_wilcoxon.pdf'),7,5)
for(i in 1:length(ortho_EpT)){
